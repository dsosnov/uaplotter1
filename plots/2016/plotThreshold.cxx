{
  ...
  { //! For CaloTower
    TString histoName="CMScalo/calotower_e_eta_h[";
    TString name="Calo Towers";
    hN = histoName; hN+=0; hN+="]";
    TH2F *calotower_e_eta_h = (TH2F*) f->Get(hN.Data());
    calotower_e_eta_h->SetTitle(name.Data());
    c1->SetLogy(); c1->SetLogz();
    calotower_e_eta_h->Draw("COLZ");
    c1->SaveAs("calotower_e_eta.eps");
    c1->Clear();
  }
  { //! For CaloTower
    TString histoName="CMSpf/pf_e_eta_h[";
    TString name="Particle Flow";
    hN = histoName; hN+=0; hN+="]";
    TH2F *pf_e_eta_h = (TH2F*) f->Get(hN.Data());
    pf_e_eta_h->SetTitle(name.Data());
    c1->SetLogy(); c1->SetLogz();
    pf_e_eta_h->Draw("COLZ");
    c1->SaveAs("pf_e_eta.eps");
    c1->Clear();
  }
  { //! For ParticleFlow summary
    std::vector<TString> histoNames={"CMSpf/pf_e_eta_h[", "CMSpf/pf_e_hch_h[", "CMSpf/pf_e_h0_h[", "CMSpf/pf_e_em0_h["};
    std::vector<TString> names={"Particle Flow", "Charged hadrons", "h0", "gamma"};
    c1->SetLogy(); c1->SetLogz();
    c1->Divide(2,2);
    ...
    c1->cd();
    c1->SaveAs("pf_e_summary.eps");
    c1->Clear();
    histoNames.clear(); hs.clear();
  }
  { //! For definition of thresholds
    const double PERCENT_OF_ENTRIES = 99.7 / 100;
    // std::vector<TString> histoNames = {"CMScalo/calotower_e_eta_h", "CMSpf/pf_e_eta_h"};
    // std::vector<TString> namesForHisto = {"Calo Towers", "Particle Flow"};
    // std::vector<int>     colors =        {4, 2};
    std::vector<TString> histoNames = {"CMSpf/pf_e_eta_h", "CMScalo/calotower_e_eta_h"};
    std::vector<TString> namesForHisto = {"Particle Flow", "Calo Towers"};
    ...
    { //! Calculating of thresholds and printing it
      for (int i = 0; i<histoNames.size(); ++i){
        ...
        for(int b = 1; b<=nbins; b++){ // Finding threshold for bin
          ...
          std::cout << "For histogram: " << h << ", bin: " << b << " threshold at " << threshold <<" [ " << sum << " at bin " << currentBin << " ]" << std::endl;
    ...
    { //! printing comparing CaloTowers and PF
      ...
      for (int i = 0; i<histoNames.size(); ++i){
        ...
        threshold_h->SetAxisRange(0, 45,"Y");
      ...
      leg->Draw();
      c3->Update();
      c3->SaveAs("thresolds_comparing.eps");
      c3->Clear();
    }
  }
  f->Close();
