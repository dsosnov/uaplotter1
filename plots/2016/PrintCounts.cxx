...
void PrintCounts(string folder="", bool recalculate = true){
  struct th_param{
  ...
  if(folder==string("") && recalculate){
    cout << "Empty folder string! Set recalculate to false\n";
    recalculate=false;
  }
  if(recalculate){
    TChain *t = new TChain();
    auto countFiles = t->Add((folder+string("/*_9*.root/hf_by_processID")).c_str());
    ...
    for(auto thr: thresholds){
      ...
      for(int type = 1; type < 5; type++){
        ...
        param.NCorrect_of_all_this_type.push_back(make_pair(double(countByType[type])/double(countTotal[type]),
                                                            GetErrorOfDivision(countByType[type], countByType_errors[type],
                                                                               countTotal[type], countTotal_errors[type])));
        std::cout << "    => purity     = " << purity << " ± " << purityError << std::endl;
        std::cout << "       efficiency = " << efficiency  << " ± " << efficiencyError << std::endl;
        // std::cout << "       N_triggeret_correctly / N_all_process = " << double(countByType[type])/double(countTotal[type]) << std::endl;
      }
  ...
  }
  {
    ...
    if(recalculate){
      ...
      purityMinus = new TH1F("PurityMinus","N_{SD-}/N_{Veto-} (purity for SD---);Threshold", 190, 1, 20);
      efficiencyMinus = new TH1F("EfficiencyMinus","N_{Veto-}/N_{All} (efficiency for SD---);Threshold", 190, 1, 20);
      purityPlus = new TH1F("PurityPlus","N_{SD+}/N_{Veto+} (purity for SD+);Threshold", 190, 1, 20);
      efficiencyPlus = new TH1F("EfficiencyPlus","N_{Veto+}/N_{All} (efficiency for SD+);Threshold", 190, 1, 20);
      ...
      efficiencyFullMinus->SetTitle("Efficiency_{HF-} * (1 - P_{Fake-}); thresholds");
      efficiencyFullPlus->SetTitle( "Efficiency_{HF+} * (1 - P_{Fake+}); thresholds");
      ...
      efficiencyOutMinus->SetTitle("Efficiency_{HF-} * (1 - P_{Fake-}) * purity_{HF-}; thresholds");
      efficiencyOutPlus->SetTitle("Efficiency_{HF+} * (1 - P_{Fake+}) * purity_{HF+}; thresholds");
    }else{
    ...
    canv->Clear();
    {
      auto hs3 = new THStack ("Purity&Efficiency_full",";Threshold");
      ...
      leg3->Draw();
      canv->SetLogy(1);
      canv->SaveAs("purity_vs_full_efficiency_log.eps");
      canv->SetLogy(0);
      canv->SaveAs("purity_vs_full_efficiency.eps");
      leg3->Clear();
    };
    canv->Clear();
    {
