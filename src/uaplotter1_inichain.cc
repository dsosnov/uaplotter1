...
std::vector<std::string> filesPARun2016_pPb_noise = {"uatree.root"};
std::vector<std::string> filesPARun2016_Pbp_noise = {"uatree.root"};
std::vector<std::string> filesPARun2016_pPb = {"uatree.root"};
std::vector<std::string> filesPARun2016_Pbp = {"uatree.root"};
// also sets zdc56
  ...
  switch (mc) {
    ...
    case -3: {
      ...
      if (ppb) {
        ...
        for (auto s: filesPARun2016_pPb_noise) chainTree->Add(s.c_str());
       } else {
        str = "noise_PARun2016_Pbp";
        // chainTree->Add("/afs/cern.ch/work/d/dsosnov/pA16/CMSSW_8_0_26/src/test2016data.root");
        for (auto s: filesPARun2016_Pbp_noise) chainTree->Add(s.c_str());
      }
      break;
     };
    // NOISE ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    ...
    case 0: { // DATA ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
      ...
      if (ppb) {
        ...
        for (auto s: filesPARun2016_pPb) chainTree->Add(s.c_str());
      } else {
         str = "data_PARun2016_Pbp";
         // chainTree->Add("/afs/cern.ch/work/d/dsosnov/pA16/CMSSW_8_0_26/src/test2016data.root");
         for (auto s: filesPARun2016_Pbp) chainTree->Add(s.c_str());
      }
      break;
    };
